...
프로덕션에서 필수에 가까운 것은 `APP_SECRET_KEY`, `CRYPTO_KEY`, `GOOGLE_CLIENT_SECRETS_FILE`, `GOOGLE_REDIRECT_URI`입니다. 나머지는 기본값으로도 기동할 수 있습니다.
...
Render 기본 **Python 네이티브 런타임**에는 Chromium/Chrome이 포함되지 않으며, Selenium용 브라우저 스택을 안정적으로 유지하는 것은 공식 지원 범위를 벗어난 경우가 많습니다. 이 앱은 **Google OAuth**, **정적 UI**, **Moodle ICS(iCal) 동기화**, **클라이언트 가져오기** 등 브라우저 없이 동작하는 경로가 있습니다. **eTL 전용 Selenium 스크래핑**은 로컬, Docker 이미지(Chromium + 드라이버 포함), 또는 별도 워커 인스턴스에서 실행하는 것을 권장합니다. 클라우드 Web Service에서만 eTL 브라우저 동기화를 기대하면 Chrome 미설치 등으로 실패할 수 있습니다.
...
`.env.example`을 복사해 `.env`를 만들고 값을 채웁니다. Google 관련 상세 주석은 `.env.example`에 있습니다.
